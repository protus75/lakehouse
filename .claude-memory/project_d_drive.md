...
**How to apply:** When adding any new storage paths (model caches, databases, volumes, temp files), always use D drive paths. Check for anything defaulting to C drive (e.g. `~/.ollama`, `~/.cache`, Docker named volumes, tool temp dirs) and redirect to D drive. This includes Claude Code temp files (`CLAUDE_TMPDIR=D:\Claude\Temp`). Key locations:
...
- Claude Code temp: `D:\Claude\Temp` (via CLAUDE_TMPDIR in .claude/settings.local.json)
